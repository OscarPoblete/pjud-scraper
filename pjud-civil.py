...
with open("TRIBUNALES_civil.json", "r", encoding="utf-8") as file: #lee el archivo TRIBUNALES.json generado por pjud-tribunales.py
...
try:
    wb = load_workbook("Estados_diarios_CIVIL.xlsx")
    ws = wb.active
...
for tribunal in tribunales:
    ...
    for fecha in fechas:
        ...
        table_civil = soup.find("table", id="data-table-estado-diario-civil")
        table = soup.find("table", id="data-table-estado-diario") #cuando hay una unica tabla en el tribunal
        
        if table_civil:
            print(f"  - Fecha: {fecha} - OK")
            # Extract table headers
            headers = [header.text.strip() for header in table_civil.find_all("th")]

            # Extract table rows
            for row in table_civil.find_all("tr")[1:]:  # Skip header row
