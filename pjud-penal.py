...
try:
    wb = load_workbook("Estados_diarios_PENAL.xlsx")
    ws = wb.active
...
for tribunal in tribunales:
    ...
    for fecha in fechas:
        ...
        table_penal = soup.find("table", id="data-table-estado-diario-penal")
        table = soup.find("table", id="data-table-estado-diario") #cuando hay una unica tabla en el tribunal
        
        if table_penal:
            print(f"  - Fecha: {fecha} - OK")
            # Extract table headers
            headers = [header.text.strip() for header in table_penal.find_all("th")]

            # Extract table rows
            for row in table_penal.find_all("tr")[1:]:  # Skip header row
